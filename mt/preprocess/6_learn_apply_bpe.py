...
for dataset in datasets:
    ...
    # Create path
    savepath = os.path.join(dataset, DATASET_BPE_NAME)
    Path(savepath).mkdir(parents=True, exist_ok=True)

    # Learn and apply BPE
    subprocess.call(['sh', './scripts/2_learn_bpe.sh', str(VOCAB_SIZE), src, trg, dataset, FASTBPE_PATH])
    subprocess.call(['sh', './scripts/2_apply_bpe.sh', str(VOCAB_SIZE), src, trg, dataset, FASTBPE_PATH])
