...
def run_experiment(datapath, src, trg, alpha, domain=None, smart_batch=False):
    ...
    fit(model1, model2, optimizer, train_loader=train_loader, val_loader=val_loader,
...
def fit(model1, model2, optimizer, train_loader, val_loader, epochs, criterion, checkpoint_path, tb_writer=None):
    ...
    for epoch_i in range(epochs):
        ...
        # Evaluate
        val_loss, translations = evaluate(model2, val_loader, criterion)

        # Log progress
        metrics = log_progress(epoch_i, start_time, tr_loss, val_loss, tb_writer, translations, print_translations=True)

        # Checkpoint
        new_best_score = save_checkpoint(model2, checkpoint_path, metrics, best_score)
...
def train(model1, model2, optimizer, data_loader, criterion):
    ...
    for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
        try:
            ...
            # Compute loss
            loss = criterion(output, trg2) / ACC_GRADIENTS  # Normalize loss
            loss.backward()
...
def log_progress(epoch_i, start_time, tr_loss, val_loss, tb_writer=None, translations=None, print_translations=True):
    metrics = {
        "train": {
            "loss": tr_loss,
